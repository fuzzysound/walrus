...
public class DepositTrackingTaskManagerTest extends TaskManagerTest {
    @Autowired private DepositTrackingTaskManager depositTrackingTaskManager;
    ...
    public void whenTrackFirstTime() {
        // when
        depositTrackingTaskManager.run();
    ...
    public void whenTrackInMiddle() {
        ...
        lastConfirmedBlockNumberEntity.setDistKey(WalrusTaskManagerKey.DEPOSIT_TRACKING.name());
        ...
        List<DepositTrackingTaskSpec> taskSpecList = extractTaskSpecList(taskEntitiyList);
    ...
    private List<DepositTrackingTaskSpec> extractTaskSpecList(List<TaskEntity> taskEntityList) {
        return taskEntityList.stream()
                .map(taskEntity -> {
                    try {
                        return objectMapper.readValue(taskEntity.getTaskSpec(), DepositTrackingTaskSpec.class);
