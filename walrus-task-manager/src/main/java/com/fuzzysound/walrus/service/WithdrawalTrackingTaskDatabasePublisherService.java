...
public class WithdrawalTrackingTaskDatabasePublisherService
        implements WithdrawalTrackingTaskPublisherService {
    ...
    private void createAndPublishTask(BigInteger blockNumber) {
        ...
        List<WalrusTransaction> withdrawalTransactions = walletService.filterTransactionWithdrawFromWalrus(block.getTransactions());
        withdrawalTransactions.stream().map(transaction -> TaskUtils.createWithdrawalTrackingTask(block, transaction))
